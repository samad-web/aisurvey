  ...
  async create(input: SurveyResponseInput, meta: InsertMeta): Promise<{ id: string }> {
    ...
    const rows = await sql<{ id: string }[]>`
      insert into survey_responses (
        ...
        ai_usage, ai_tools, stop_reason, ai_wants,
        spend, will_pay, pricing_model,
        ...
        ${input.practitionerStatus}, ${input.barCouncil ?? null}, ${input.barEnrollmentNumber ?? null},
        ${input.institution ?? null}, ${input.course ?? null},
